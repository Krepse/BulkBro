...
export function WorkoutDetailsView({ workout, onNavigate, onEdit, onDelete, onUpdate, allWorkouts, onViewStats }: WorkoutDetailsViewProps) {
    ...
    // Guard: Prevent infinite Strava fetch loop.
    // saveWorkout() regenerates exercise/set UUIDs on every save, which would
    // change the workout prop and re-trigger this effect endlessly.
    const hasFetchedStravaRef = useRef(false);
    const workoutRef = useRef(workout);
    workoutRef.current = workout;

    // Reset the fetch guard when navigating to a different workout
    useEffect(() => {
        hasFetchedStravaRef.current = false;
    }, [workout.id]);
    ...
    useEffect(() => {
        // Already fetched for this workout — don't re-fetch
        if (hasFetchedStravaRef.current) return;

        const currentWorkout = workoutRef.current;

        // 1. Check for Cached Analysis AND Validity
        if (currentWorkout.stravaAnalysis) {
            // Validate that the cached keys actually match the current set IDs
            const currentSetIds = new Set(currentWorkout.ovelser.flatMap(e => e.sett.map(s => String(s.id))));
            const cachedSetIds = Object.keys(currentWorkout.stravaAnalysis.setStats || {});

            const idsMatch = cachedSetIds.length === 0 || cachedSetIds.some(id => currentSetIds.has(id));
            const hasCalories = (currentWorkout.stravaAnalysis.workoutStats?.calories || 0) > 0;

            const isCacheValid = idsMatch && hasCalories;

            if (isCacheValid) {
                console.log("Using cached Strava analysis");
                setExerciseStats(currentWorkout.stravaAnalysis.exerciseStats);
                setSetStats(currentWorkout.stravaAnalysis.setStats);
                setWorkoutStats(currentWorkout.stravaAnalysis.workoutStats);
                setStravaActivity({ id: currentWorkout.stravaAnalysis.activityId });
                hasFetchedStravaRef.current = true;
                return;
            } else {
        ...
        // 2. Fetch if connected and no valid cache
        if (stravaConnected && currentWorkout.startTime) {
            hasFetchedStravaRef.current = true; // Mark BEFORE async to prevent double-fire
            const fetchStravaData = async () => {
                setIsFetchingStrava(true);
                try {
                    const endTime = currentWorkout.endTime || new Date(new Date(currentWorkout.startTime!).getTime() + 90 * 60000).toISOString();

                    const activity = await findOverlappingActivity(currentWorkout.startTime!, endTime);

                    if (activity) {
                        setStravaActivity(activity);
                        const streams = await getActivityStreams(activity.id);
                        if (streams) {
                            const stats = calculateDetailedStats(currentWorkout, activity, streams);
                            if (stats) {
                                setExerciseStats(stats.exerciseStats);
                                setSetStats(stats.setStats);
                                setWorkoutStats(stats.workoutStats);

                                // PERSIST CACHE — use workoutRef for latest data
